...
def save_mat(flag,path,feat,h_list,im_names):
  """
  Args:
    flag:if 1,first to write,if 0 ,need to add to the mat
    path:mat file path
    feat:n*24*8*1
  ...
  """
  if flag == 1:
    result = {'feat':feat, 'h_list':h_list, 'im_names':im_names}
    sio.savemat(path,result)
  else:
    result = sio.loadmat(path)
    result['feat']=np.concatenate(result['feat'],feat)
    #result['g_list']=np.concatenate(result['g_list'],g_list)
    result['h_list']=np.concatenate(result['h_list'],h_list)
    result['im_names']=np.concatenate(result['im_names'],im_names)
    sio.savemat(path,result)
