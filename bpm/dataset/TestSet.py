...
from ..utils.distance import compute_dist,compute_dist2
...
class TestSet(Dataset):
  ...
  def extract_feat(self, normalize_feat, verbose=True):
    ...
    feat = np.vstack(feat)
    #print('feat vstack:{}'.format(feat.shape))
    ids = np.hstack(ids)
    cams = np.hstack(cams)
  ...
  def eval(
    ...
    with measure_time('Extracting feature...', verbose=verbose):
     ...
     # results = {'feat':feat,'ids':ids,'cams':cams,'im_names':im_names,'marks':marks}
      #sio.savemat('/GPUFS/nsccgz_ywang_1/alice/workspace/beyond-part-models/test/test.mat',results)
