...
from bpm.utils.dataset_utils import anchor_positive_negative
...
def save_images(original_file, save_dir=None, train_test_split_file=None):
  ...
  anchor_positive_negative(im_paths, parse_original_im_name, save_dir)
...
def transform(original_file, save_dir=None):
  ...
  apn_pkl_file = osp.join(save_dir, 'anchor_positive_negative_split.pkl')
  ...
  new_anchor_names = change_im_names(
    anchor_list, parse_original_im_name, new_im_name_tmpl)
  new_anchor_array = np.array(new_anchor_names)
  new_positive_names = []
  new_negative_names = []
  for i in range(len(positive_list)):
    p=positive_list[i]
    n=negative_list[i]
    pt=np.where(anchor_array==p)
    nt=np.where(anchor_array==n)
    ptt=pt[0][0]
    ntt=nt[0][0]
    new_p=new_anchor_names[ptt]
    new_n=new_anchor_names[ntt]
    new_positive_names.append(new_p)
    new_negative_names.append(new_n)
  # mess up the order of anchor_positive_negative (trainval)
  new_tv_anchor_names, new_tv_positive_names, new_tv_negative_names = mess_up_apn(new_anchor_names, new_positive_names, new_negative_names)
  # select the train anchor_positive_negative
  train_anchor_names = []
  train_positive_names = []
  train_negative_names = []
  for i in range(train_im_names.shape[0]):
    k = np.where(new_anchor_array==train_im_names[i])
    ind = k[0][0]
    ta = new_anchor_names[ind]
    tp = new_positive_names[ind]
    tn = new_negative_names[ind]
  ...
  partition_file = osp.join(save_dir, 'new_shuffle_partitions.pkl')
